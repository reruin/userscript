...
function coreInsert(wrapper , onSuccess){
  var script = function(){
    var origin_update_page = window.update_page
    var rules = []
    ...
    function notify(msg,title,stop){
    ...
    var dig = (function($){
      ...
      function init(){
        ...
        var defaultRule = [
          '# 参数：cpu(C)/内存(M)/硬盘(G)/地域/价格/溢价率(R)',
          '# 估值 = cpu * 2.5 + ram每GB * 5 + disk每10GB * 2 + (ip数-1)*2.5',
          '# 溢价率 = 估值 / 原值 - 1',
          '# 地域: LA/SJ/SEA/PHX/DAL/BUF/PIS/CHI',
          '# 示例：价格20以内，核心2~4，内存>=3000M，硬盘至少20G，流量至少1T,仅限CA',
          '# 0-20/2-4C/3000M/20G/1T/CA',
          '',
          '4R/0-10/1000M/10G/1T',
          '0-2',
          '1.6R/0-5/100G/0.5T',
          '1.8R/0-7/1T/CA/AZ/AZ1/10G'
          ].join('\r')
        ...
        window.update_page = function(){
            //process(data)
            //if(origin_update_page) origin_update_page()
      ...
      function updateDom(data){
        $('div.section_row').show(); $('div#error').hide();
        if($('div#planid').text() == data['planid'] && !data['ended']) return;
        $('h4#pricing').html(data['price']);
        $('strong#virt').text(data['virt']);
        $('span#message').text(data['message']);
        if(data['windows']=="TRUE") { $('li#windows').show() } else { $('li#windows').hide() };
        $('strong#ram').text(data['ram'] + "MB");
        $('strong#cpu').text(data['cpu']);
        $('strong#hdd').text(data['hdd'] + "GB");
        $('strong#bw').text(data['bw'] + "GB");
        $('strong#location').text(data['location']);
        $('strong#ips').text(data['ips']);
        $('a#buyurl').attr('href', 'https://billing.virmach.com/cart.php?a=add&pid='+data['pid']+'&billingcycle=annually').attr('style', '').text("ORDER NOW");
        if(data['ended']) {
          $('a#buyurl').removeAttr('href').css('background-color', 'red').text("SWITCHING SOON");
          return;
        }
        if($('div#planid').text() != "") sendNotification(data); // Don't send if this is the first plan.
        $('div#planid').text(data['planid']);
        if(data.planid != lastPlanId){
          let msg = (`${data.price}\r${data.virt}\r${data.cpu}C / ${data.ram}M / ${data.hdd}GB SSD / ${data.bw}GB / ${data.ips} ip / ${data.location}`)
